...
var MAX_SOLUTIONS_COUNT = COL_ROW[1] * COL_ROW[0] * $('#form_max_paths').val();
...
function create_empty_board() {
  var result = new Array(COL_ROW[1]);
  for (var i = 0; i < COL_ROW[1]; ++ i) {
    result[i] = new Array(COL_ROW[0]);
  }
  return result;
}
  ...
  for (var i = 0; i < COL_ROW[1]; ++ i) {
    for (var j = 0; j < COL_ROW[0]; ++ j) {
      ...
      while (stack.length) {
        ...
        if (n.row < COL_ROW[1]-1) { stack.push(make_rc(n.row+1, n.col)); }
        if (n.col > 0) { stack.push(make_rc(n.row, n.col-1)); }
        if (n.col < COL_ROW[0]-1) { stack.push(make_rc(n.row, n.col+1)); }
      }
      var isRow = false;
      for(var k = 0; k < COL_ROW[1]; ++k)
      {
        if (COL_ROW[1] == "4") {
...
function in_place_drop_empty_spaces(board) {
  for (var j = 0; j < COL_ROW[0]; ++ j) {
    var dest_i = COL_ROW[1]-1;
    for (var src_i = COL_ROW[1]-1; src_i >= 0; -- src_i) {
      if (board[src_i][j] != 'X') {
...
function can_move_orb(rc, dir) {
  switch (dir) {
    case 0: return                    rc.col < COL_ROW[0]-1;
    case 1: return rc.row < COL_ROW[1]-1 && rc.col < COL_ROW[0]-1;
    case 2: return rc.row < COL_ROW[1]-1;
    case 3: return rc.row < COL_ROW[1]-1 && rc.col > 0;
    case 4: return                    rc.col > 0;
    case 5: return rc.row > 0      && rc.col > 0;
    case 6: return rc.row > 0;
    case 7: return rc.row > 0      && rc.col < COL_ROW[0]-1;
  }
  return false;
}
...
function get_max_path_length() {
  MAX_SOLUTIONS_COUNT = COL_ROW[1] * COL_ROW[0] * $('#form_max_paths').val();
  return $('#form_max_length').val();
}
...
//   }
//   if (!enable) {
//     $(el).attr('unselectable','on').css({'-moz-user-select':'-moz-none',
...
//   } else {
//     $(el).attr('unselectable','off').css({'-moz-user-select':'text',
